 ...
 * 리포트에 실릴 해석 문구.
 *
 * ⚠ **리포트 조립은 이제 이 함수를 쓰지 않는다.** 문구가 학년 구분 없이 박혀 있어 초3과
 *   중2가 같은 글을 받고 있었고, 고치려면 배포를 해야 했다. 지금은 리포트 관리 화면이
 *   고치는 값에서 온다 — lib/reportAssets.ts(씨앗)와 lib/reportAssetStore.ts(덮어쓰기).
 *   여기 문장은 그 씨앗의 출처로 남겨 두되, 고칠 일이 생기면 저쪽을 고친다.
 *
