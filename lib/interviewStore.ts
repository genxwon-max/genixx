...
/* ───────────────────────── 날짜·시각 ─────────────────────────
   산술 자체는 lib/calendar.ts로 옮겼다 — 회원 존의 면담 예약 화면이 같은 달력을 쓰는데,
   이 파일을 부르면 전문가 콘솔의 케이스 자료가 통째로 딸려 오기 때문이다.

   이름을 그대로 내보낸다. 이 콘솔의 화면 열댓 곳이 여기서 dayNum·monthCells를 가져다
   쓰고 있어, 수입처만 바꾸면 그 줄을 전부 고쳐야 한다 — 달력이 어디 있는지는 화면이
   알아야 할 일이 아니다. */
export {
  pad,
  dayNum,
