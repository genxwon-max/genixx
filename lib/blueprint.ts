...
/**
 * 출제위원이 제출 전에 전 항목을 짚고 서명한다. 하나라도 비면 제출하지 못한다.
 *
 * §9의 열 항목을 갈아 끼웠다. id를 옛 것(code · verb · distractor …)과 겹치지 않게
 * 새로 붙인 것은, 옛 목록에서 짚어 둔 id가 저장분에 남아 있어서다 — 같은 id를 쓰면
 * 「S1 연산 없음」에 짚은 것이 「S4 정답형」까지 묻는 새 문장에 짚은 것으로 읽힌다.
 *
 * 자동으로 켜는 항목은 두지 않는다. 기계가 가릴 수 있는 것(코드 형식 · 배점 매핑 …)도
 * 목록 안에서 경고만 띄우고, 짚는 것은 사람이 한다 — NCIC 대조처럼 기계가 못 보는 것이
 * 같은 문장 안에 붙어 있다.
 */
export const submitChecklist: CheckItem[] = [
  ...
  {
    id: "chk-verb",
    text: "인지 동사가 단계 정의와 일치하는가? (S1 연산·추론 없음 / S3 ‘왜’ 없음 / S4 정답형)",
  },
  {
    id: "chk-format",
    text: "형식·배점이 고정 매핑과 일치하는가? (S1·S2 선택 1점 / S3 단답 2점 / S3·S4 서술·논술 3점 — 0.5점 없음)",
  },
  {
    id: "chk-distractor",
    text: "모든 오답지가 특정 오개념을 의도하는가? 최장 보기=정답 등 길이 단서가 없는가?",
  },
  {
    id: "chk-condition",
  ...
  {
    id: "chk-perspective",
    text: "문항마다 ⑫ 재능 평가 관점(인지 처리 위계 구체 / 인지 능력 관련 수준)이 빠짐없이 기재되었는가?",
  },
  { id: "chk-time", text: "한 문항의 풀이 시간이 2분 이내인가?" },
];
