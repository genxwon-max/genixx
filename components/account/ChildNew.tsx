 ...
 * 학교급과 그 안의 학년.
 *
 * 평가가 열리는 구간은 초등 1학년 ~ 중학교 3학년이지만, 학교급은 초·중·고 셋을 다 받는다.
 * 형제자매를 한 계정에 모아 두는 일이 흔한데 큰아이가 고등학생이라고 명부에 올리지도
 * 못하면 보호자는 아이마다 다른 자리를 찾아야 한다.
 *
 * ⚠ 고등학생은 아직 접수할 평가가 없다. 차림표(lib/examCatalog.ts)가 초1~중3 학년마다
 *   여는데 고등학교 칸이 없어 「내 학년」으로 걸리는 카드가 없다. 설문은 나간다 —
 *   학년대(lib/surveyBands.ts)가 고등학생을 가장 위 칸(중2~3)으로 받는다.
 *
 * ⚠ 설문 학년대는 초등 3~4학년이 가장 아래다(lib/surveyBands.ts). 초1·2 학생에게는 그
 *   칸의 문항이 나간다 — 평가가 초1까지 내려온 만큼 설문 문항도 아래 칸을 따로 둘지
 *   정해야 한다. 정해지면 저 파일 한 곳을 고친다.
 */
const schoolLevels = [
