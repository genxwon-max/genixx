 ...
 *
 * 표에 적히는 것만 여기서 정한다. 남는 것은 학년 넷과 교과 셋, 곧 **누를 수 있는 칸**뿐이다.
 *
 * 이름과 ID 칸은 세우지 않는다. 가입해야 생기는 값이라 무엇을 적어도 거짓이 된다 —
 * 비워 두면 채우라는 말로 읽히고, 「가입 후」라고 적으면 가입하라는 재촉이 표 한가운데에
 * 선다. 아직 없는 것은 묻지 않는 편이 낫다. 가입한 뒤 받는 시험지(ExamSession의 StartGate)
 * 에서는 같은 자리에 이름과 접속코드가 실제로 채워진다.
 *
